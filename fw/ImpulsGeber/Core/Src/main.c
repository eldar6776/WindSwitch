...
uint32_t pulseDelay = 100; // Početna vrednost (ms), biće postavljena prema DIP switch-u
const uint32_t dip_to_period[64] = {
    0, 500, 250, 125, 77, 50, 33, 25, 20, 16, 14, 12, 11, 10, 8, 7, 
...
int main(void)
{
    ...
    while(1)
    {
        // Povremeno ažuriraj vrednost iz DIP switch-a
        static uint32_t lastDipRead = 0;
        if (HAL_GetTick() - lastDipRead >= 500) {  // Čitaj DIP na svakih 500ms
            lastDipRead = HAL_GetTick();
            pulseDelay = ReadDIP();
        }
        if (pulseDelay && (HAL_GetTick() - previousTick >= pulseDelay)) {
            previousTick = HAL_GetTick(); // Ažuriraj referentni trenutak
            HAL_GPIO_TogglePin(DOOR_LOCK_GPIO_Port, DOOR_LOCK_Pin); // Toggle impulsa
            HAL_GPIO_TogglePin(LED_STATUS_GPIO_Port, LED_STATUS_Pin); // Toggle impulsa
        }
        /* USER CODE END WHILE */
